<template>
  <div>
    <MDBContainer>
      <div class="container">
        <div class="row mb-5 pb-5">
          <div class="col-md-4 mb-1">
            <MDBCard class="shadow">
              <MDBCardImg
                src="https://mdbootstrap.com/img/new/standard/nature/184.jpg"
                top
                alt="..."
              />
              <MDBCardBody>
                <MDBCardTitle>Card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
                <MDBBtn tag="a" href="!" color="primary">Button</MDBBtn>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-4 mb-1 pb-1">
            <MDBCard class="shadow">
              <MDBCardImg
                src="https://mdbootstrap.com/img/new/standard/nature/185.jpg"
                top
                alt="..."
              />
              <MDBCardBody>
                <MDBCardTitle>Card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
                <MDBBtn tag="a" href="!" color="primary">Button</MDBBtn>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-4 mb-1 pb-1">
            <MDBCard class="shadow">
              <MDBCardImg
                src="https://mdbootstrap.com/img/new/standard/nature/186.jpg"
                top
                alt="..."
              />
              <MDBCardBody>
                <MDBCardTitle>Card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
                <MDBBtn tag="a" href="!" color="primary">Button</MDBBtn>
              </MDBCardBody>
            </MDBCard>
          </div>
        </div>
      </div>
    </MDBContainer>
    <MDBContainer>
      <div class="container">
        <center>
          <div class="row mb-5 pb-5 ">
            <div class="col-md-3 mb-4">
              <MDBCard class="shadow" style="width: 100%">
                <MDBCardImg
                  top
                  src="https://mdbootstrap.com/img/new/standard/nature/180.jpg"
                  alt="..."
                />
                <MDBCardBody>
                  <MDBCardText>
                    Some quick example text to build on the card title and make
                    up the bulk of the card's content.
                  </MDBCardText>
                </MDBCardBody>
              </MDBCard>
            </div>
            <div class="col-md-3 mb-4">
              <MDBCard class="shadow" style="width: 100%">
                <MDBCardImg
                  top
                  src="https://mdbootstrap.com/img/new/standard/nature/181.jpg"
                  alt="..."
                />
                <MDBCardBody>
                  <MDBCardText>
                    Some quick example text to build on the card title and make
                    up the bulk of the card's content.
                  </MDBCardText>
                </MDBCardBody>
              </MDBCard>
            </div>
            <div class="col-md-3  mb-4">
              <MDBCard class="shadow" style="width: 100%">
                <MDBCardImg
                  top
                  src="https://mdbootstrap.com/img/new/standard/nature/182.jpg"
                  alt="..."
                />
                <MDBCardBody>
                  <MDBCardText>
                    Some quick example text to build on the card title and make
                    up the bulk of the card's content.
                  </MDBCardText>
                </MDBCardBody>
              </MDBCard>
            </div>
            <div class="col-md-3 mb-4">
              <MDBCard class="shadow" style="width: 100%">
                <MDBCardImg
                  top
                  src="https://mdbootstrap.com/img/new/standard/nature/183.jpg"
                  alt="..."
                />
                <MDBCardBody>
                  <MDBCardText>
                    Some quick example text to build on the card title and make
                    up the bulk of the card's content.
                  </MDBCardText>
                </MDBCardBody>
              </MDBCard>
            </div>
          </div>
        </center>
      </div>
    </MDBContainer>
    <MDBContainer>
      <div class="container mb-5">
        <div class="row">
          <div class="col-md-3 mb-3">
            <MDBCard
              shadow="0"
              border="primary"
              text="primary"
              bg="white"
              class="mb-3"
            >
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Primary card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard text="white" bg="secondary" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Secondary card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard text="white" bg="success" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Success card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard text="white" bg="danger" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Danger card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard bg="warning" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Warning card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard text="body" bg="info" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Info card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard bg="light" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Light card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
          <div class="col-md-3 mb-3">
            <MDBCard text="white" bg="dark" class="mb-3">
              <MDBCardHeader>Header</MDBCardHeader>
              <MDBCardBody>
                <MDBCardTitle>Dark card title</MDBCardTitle>
                <MDBCardText>
                  Some quick example text to build on the card title and make up
                  the bulk of the card's content.
                </MDBCardText>
              </MDBCardBody>
            </MDBCard>
          </div>
        </div>
      </div>
    </MDBContainer>
  </div>
</template>

<script>
import {
  MDBCard,
  MDBCardBody,
  MDBCardTitle,
  MDBCardText,
  MDBCardImg,
  MDBBtn,
  MDBCardHeader,
} from "mdb-vue-ui-kit";
export default {
  name: "Card",
  components: {
    MDBCard,
    MDBCardBody,
    MDBCardTitle,
    MDBCardText,
    MDBCardImg,
    MDBBtn,
    MDBCardHeader,
  },
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped></style>
